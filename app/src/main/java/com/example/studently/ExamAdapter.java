...
public class ExamAdapter extends BaseAdapter {
    ...
    private List<ExamModel> posts;
    private Context context;

    ExamAdapter(Context context) {
        this.context = context;
        posts = ExamModel.getAllExams();

        //sort list by time
        Collections.sort(posts, new ExamModel.CustomComparator());
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String date = ExamModel.getDate(posts.get(position));
        String time1 = ExamModel.getTime(posts.get(position));
